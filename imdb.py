...
class IMDB_Classifier(object):
    ...
    def __init__(self):
        ...
        self.nodes_per_layer = [128, 128, 128]
    ...
    def evaluate(self):
        ...
        y_pred_prob = normalize(y_pred_prob, 0, 1)
        y_pred = y_pred_prob > self.threshold
        y_pred = y_pred * 1
        y_true = y_test

        from helpers import get_clf_report
        cm, report, acc, prec, recall, auc = get_clf_report(
...
if __name__ == '__main__':
    ...
    clf = IMDB_Classifier()
    clf.train(lr=1e-2, epochs=100)
    clf.continue_train(lr=1e-3, epochs=100, resume_model='last')
    clf.evaluate()
    del (clf)
